	...
	<script type="text/javascript">
		$(function(){
			...
			$("#selectPage").change(function(){
	        	...
	        	if("${type}"=="list"){
	        		window.location.href='${pageContext.request.contextPath}/commonlyusedquery/driving/'+index;
	        	}
		        ...
		        <TR>
		            ...
		            <TD vAlign=top width="100%" bgColor=#ffffff>
		                <TABLE cellSpacing=0 cellPadding=5 width="100%" border=0>
			                <div class="place">
			                 	<TR>
			                        <TD class=manageHead>当前位置： &gt; 有驾照的员工 </TD>
			                    </TR>
			                    <TR>
			                     	<TD height=1></TD>
			                    </TR>
			                    ...
			                    <TR>
			                        <TD>
			                            <TABLE id=grid cellSpacing=1 cellPadding=2 rules=all border=0>
			                                ...
			                                <c:forEach items="${pager.datas}" var="driving">
				                                <TR class="normal" style="FONT-WEIGHT: normal; FONT-STYLE: normal; BACKGROUND-COLOR: white; TEXT-DECORATION: none">			                                    
				                                   	                                    
				                                    <td>${driving.emp_id}</td>
													<td>${driving.emp_name}</td>
													<td>${driving.emp_department}</td>
													
													
													
													
													<td><fmt:formatDate value="${driving.certificate_deadline}"  pattern="yyyy-MM-dd"/></td>
				                   	   ...
				                   	   <SPAN id=pagelink>
				           				   <DIV style="LINE-HEIGHT: 20px; HEIGHT: 20px; TEXT-ALIGN: right">				           				 				           				   	
								                ...
								                <c:if test="${pager.pageindex!=1 }">
								    				<c:if test="${type=='list'}">
								    					[<A href="${pageContext.request.contextPath}/commonlyusedquery/driving/${pager.pageindex-1}">前一页</A>]
								    				</c:if>
								                ...
								                <c:forEach begin="1" end="${pager.pagecount}" var="i">
								                	...
								                	<c:if test="${i!=pager.pageindex}">								                		
														<c:if test="${type=='list'}">
								    						<A class="" href="${pageContext.request.contextPath}/commonlyusedquery/driving/${i}"><c:out value="${i}"/></A> 
									    				</c:if>
								                ...
								                <c:if test="${pager.pageindex!=pager.pagecount}">								                	
								                	<c:if test="${type=='list'}">
								    					[<A class="" href="${pageContext.request.contextPath}/commonlyusedquery/driving/${pager.pageindex+1}">后一页</A>]
								    				</c:if>
